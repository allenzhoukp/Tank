...
public class ScoreCounterState extends JLayeredPane {
    ...
    public static final double WAIT_SEC = 2;
     ...
     */
    public ScoreCounterState(MainFrame mainFrame, int level, PlayerState p1State, PlayerState p2State, boolean isVictory) {
        super();
    ...
    private class CounterStateTimerListener implements TimerListener {
        ...
        private void tryPopUp() {
            ...
            while (count == 0) {
                ...
                if (tankType == 4 || count != 0)
                    Sound.play("/sounds/count.wav");

                tankType++;
                if (!(tankType < 5)) {

                    //After level: clear count
                    tankType %= 5;
                    player++;
    ...
    private String getTotalText(boolean isP1, int totalScore) {
        return (isP1 ? "1" : "2") + "P" + " TOTAL: " + (totalScore == -1 ? "" : totalScore);
    }
