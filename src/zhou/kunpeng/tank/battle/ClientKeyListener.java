...
import zhou.kunpeng.tank.messages.ClientKeyOpMessage;
...
public class ClientKeyListener extends PlayerKeyListener {
    ...
    protected void appendMove(int keycode) {
        if (gameMap.isOnline() && !gameMap.isNotClient())
            synchronized (gameMap.getNetComm()) {
                gameMap.getNetComm().send(new ClientKeyOpMessage(keycode, isP1));
            }
    }
    ...
    protected void fire() {
        if (gameMap.isOnline() && !gameMap.isNotClient())
            synchronized (gameMap.getNetComm()) {
                gameMap.getNetComm().send(new ClientKeyOpMessage(KeyEvent.VK_F, isP1));
            }
    }
    ...
    protected void stopMove() {
        if (gameMap.isOnline() && !gameMap.isNotClient())
            synchronized (gameMap.getNetComm()) {
                gameMap.getNetComm().send(new ClientKeyOpMessage(KeyEvent.VK_UNDEFINED, isP1));
            }
    }
}
